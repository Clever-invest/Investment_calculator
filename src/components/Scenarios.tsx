...
export const Scenarios: React.FC<ScenariosProps> = ({ baseInput }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
      <h3 className="text-lg font-bold text-gray-800 dark:text-gray-200 mb-6">🎯 Сценарный анализ</h3>
        ...
        {scenarios.map((scenario) => (
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h4 className="text-lg font-bold text-gray-800 dark:text-gray-200">{scenario.name}</h4>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="text-sm">
                <span className="text-gray-600 dark:text-gray-400">Бюджет ремонта:</span>
                <span className="block font-semibold">{fmtMoney(scenario.input.renovationBudget)}</span>
              </div>
              
              <div className="text-sm">
                <span className="text-gray-600 dark:text-gray-400">Срок (мес):</span>
                <span className="block font-semibold">
                  {scenario.input.monthsRepair + scenario.input.monthsExposure}
                </span>
              </div>
              
              <div className="border-t-2 border-gray-200 dark:border-gray-600 pt-3 mt-3">
                <div className="text-sm mb-2">
                  <span className="text-gray-600 dark:text-gray-400">Прибыль:</span>
                  <span className={`block font-bold text-lg ${scenario.project.profit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
      ...
      <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-950/50 border border-blue-200 dark:border-blue-800 rounded-lg">
        <h4 className="font-semibold text-blue-900 dark:text-blue-200 mb-2">📝 Методика расчета сценариев:</h4>
